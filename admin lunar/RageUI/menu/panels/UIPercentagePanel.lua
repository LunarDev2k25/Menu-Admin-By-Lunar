local Percentage = {
    ...
    Text = {
        Left = { X = 45, Y = 30, Scale = 0.23 },
        Middle = { X = 215.5, Y = 30, Scale = 0.23 },
        Right = { X = 385, Y = 30, Scale = 0.23 },
    },
}
...
function RageUI.PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(4 + CurrentMenu.X + Percentage.Bar.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + Percentage.Bar.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Percentage.Bar.Width, Percentage.Bar.Height, 22, 22, 22, 255)
